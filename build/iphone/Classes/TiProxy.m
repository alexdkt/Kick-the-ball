...
//Rare exceptions to indicate a bug in the _platinotemplate code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_platinotemplate = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_platinotemplate]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.platinotemplate.module"
